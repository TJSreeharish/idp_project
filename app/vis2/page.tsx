...
function hCol(t: number) {
  ...
  c.setHSL(0.56 - t * 0.44, 1.0, 0.42 + t * 0.22);
  return c;
}
function dCol(d: number) {
  const c = new THREE.Color();
  c.setHSL(0.66 - d * 0.66, 0.92, 0.32 + d * 0.32);
  return c;
}
function pCol(v: number) {
  const c = new THREE.Color();
  if (v < 0.005) { c.set(0x030912); return c; }
  c.setHSL(0.70 - v * 0.70, 1.0, 0.12 + v * 0.58);
  return c;
}
...
function addGridHelper(g: THREE.Group) {
  const gh = new THREE.GridHelper(HALF*2, GRID_C, 0x071828, 0x071828);
...
export default function PointPillarsPipeline() {
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.FogExp2(0x020912, 0.055);
    ...
    const dir = new THREE.DirectionalLight(0x88ccff, 1.6);
    dir.position.set(5,10,5); scene.add(dir);
    const pt = new THREE.PointLight(0x00e5ff, 1.1, 28);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background:"rgba(2,9,18,0.97)", display:"flex",
      ...
      {/* ── Stage tabs ── */}
      <div style={{display:"flex",flexShrink:0,borderBottom:`1px solid ${PAL.gridLine}`,background:"rgba(2,9,18,0.95)"}}>
        {STAGES.map((s,i)=>(
          ...
          <>
            ...
            {animating && (
              <div style={{position:"absolute",inset:0,background:"rgba(2,9,18,0.6)",display:"flex",alignItems:"center",justifyContent:"center",pointerEvents:"none"}}>
                <div style={{border:`1px solid ${PAL.accent}33`,padding:"6px 22px",color:PAL.accent,fontSize:10,letterSpacing:3,background:"rgba(0,14,24,0.88)"}}>
                  TRANSFORMING...
                </div>
              </div>
            )}
...
function HUD({label,val,col=PAL.accent}:{label:string;val:string;col?:string}){
  return(
    <div style={{background:"rgba(2,6,18,0.88)",border:`1px solid ${col}18`,padding:"4px 10px",minWidth:138}}>
...
function Legend({title,items}:{title:string;items:{c:string;l:string}[]}){
  return(
    <div style={{background:"rgba(2,6,18,0.88)",border:`1px solid ${PAL.gridLine}`,padding:"7px 12px",minWidth:180}}>
...
function ArchDiagram() {
  ...
  return (
    <div style={{
      ...
      background:`radial-gradient(ellipse at 50% 30%, #031220 0%, ${PAL.bg} 70%)`,
      ...
      <div style={{marginTop:20,width:"100%",maxWidth:680}}>
        ...
        <div style={{display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap"}}>
          {["Block A · stride 1","Block B · stride 2","Block C · stride 2"].map((blk,bi)=>(
            <div key={blk} style={{
              background:"rgba(4,14,28,0.8)", border:`1px solid ${PAL.green}28`,
